...
def request(request):
    if request.method == "POST":
        ...
        email_body = render_to_string('email_template.html', {
            ...
            'date': date,
            'time': time,
            'calendar_link': calender_link,
        })
        send_mail(
            subject="Meeting requested",
            message="You mentee has requested a meeting",
            from_email = "nbaggie2027@gmail.com",
            recipient_list=["bagavati2310098@ssn.edu.in"],
...
def accept(request):
    if request.method == "GET":
        regno = request.GET.get("register")
        mycursor.execute("SELECT Email from personal where regno = '{}'".format(regno))
        result1 = mycursor.fetchall()
        email = result1[0][0]
        print(email)
    ...
    <html>
            ...
            <body>
                <h1>Meeting Requested</h1>
                <p>Your mentor has accepted the meeting request</p>
                <ul>
                    <li><strong>Date:</strong> {date}</li>
                    <li><strong>Time:</strong> {time}</li>
